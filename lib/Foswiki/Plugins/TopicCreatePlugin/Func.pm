...
sub init {
    ...
    Foswiki::Func::writeDebug(
        "- Foswiki::Plugins::TopicCreatePlugin::Func::init( $web.$topic )")
      if $debug;
    return 1;
}
...
sub handleTopicAttach {
    ...
    if ( _existAttachment( $web, $topic, $name ) ) {
        return _errorMsg( $errVar,
            "Attachment =$name= already exists in destination topic $web.$topic"
        );
    }
...
# then use the Meta object
        my $fromTopicText =
          &Foswiki::Func::readTopicText( $fromTopicWeb, $fromTopic, "", 1 );
...
sub _copyAttachment {
    ...
    Foswiki::Func::saveAttachment( $toWeb, $toTopic, $toFile,
        { file => $filePath } );

    &Foswiki::Func::writeDebug(
"- Foswiki::Plugins::TopicCreatePlugin::copyAttachment from $fromWeb/$fromTopic/$fromFile to $toWeb/$toTopic/$toFile"
    ) if $debug;
}
